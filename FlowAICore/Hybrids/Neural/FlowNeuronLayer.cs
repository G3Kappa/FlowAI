...
namespace FlowAI.Hybrids.Neural
{
    ...
    public class FlowNeuronLayer : FlowTransformer<double[], double[]>
    {
            ...
            : base(null, (i, o) => i.Length == nNeurons, 1)
        {
            ...
            var outputJ = new MergingFlowOutputJunction<double>(Neurons.Select(n => (Func<IAsyncEnumerable<double>>)(() => n.Flow())).ToArray());

            Map = inputs =>
               {
                   var arr = inputJ.ConsumeFlow(inputs.ToAsyncEnumerable())
